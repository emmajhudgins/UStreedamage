...
FIA<-read.csv('~/Desktop/OneDrive - McGill University/Grad/scripts/FIAcodes_notypos_2019.csv', stringsAsFactors = FALSE)
fia<-list()
...
hostspp<-read.csv("~/Desktop/OneDrive - McGill University/Grad/scripts/FIA_spp_sepcols_2019.csv", stringsAsFactors = F)
genera<-read.csv('~/Desktop/OneDrive - McGill University/Grad/scripts/updated_hosts_frank.csv', stringsAsFactors = F)
...
host_gen<-host_gen[-4,]
# write.csv(host_gen, file="host_sharing_total.csv", row.names=F)
host_gen<-read.csv('~/Desktop/OneDrive - McGill University/Grad/scripts/host_sharing_total.csv')
stan_est<-t(seq(1,8))
severity<-read.csv('~/Desktop/OneDrive - McGill University/Turtles/each_spp_pesthost.csv')
data2<-data2[-c(25,69),]
data2<-data2[-4,]
for (i in 1:72)
{
...
inv_dat<-read.csv('/Users/EmmaH/SASUniversityEdition/myfolders/To Emma Hudgins/old_sas_output/inv_dat_complete.csv')
allprop<-read.csv('~/Downloads/all_prop_mostdat.csv')
codematch<-read.csv('~/SASUniversityEdition/myfolders/To Emma Hudgins/old_sas_output/fia_newcodes.csv')
inv_dat$Genus<-tolower(as.character(inv_dat$genus2))
colnames(allprop)[1]<-"code.y"
allprop2<-merge(allprop,codematch[,c("sci", "code.y", "FIA..Code")], by="code.y", all.x=F, all.y=F)
...
#nonfocal_gen_small=nonfocal_gen_med=nonfocal_gen_large=0
...
saveRDS(host_gen_small, file='host_gen_small.RDS')
saveRDS(host_gen_med, file='host_gen_med.RDS')
saveRDS(host_gen_large, file='host_gen_large.RDS')
