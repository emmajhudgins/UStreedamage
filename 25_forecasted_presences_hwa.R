...
new_hwa<-read.csv('hwa_5yr_fitting_july.csv')[,3:12]
...
LLfit=function(par)
{
  ...
  for (time in 1:(floor(YEAR/5)+9))
  {
    ...
    {
      cccc<-which(!(prez[1:length(which(prez[,spp]!=0)),spp]%in%new_hwa[1:length(which(new_hwa[,4]!=0)),4]))
      Pnext[cccc]<-0  
    }
    if (time==floor(YEAR/5))
    {
...
for (i in which(data2$COLUMN_NAM=="HWA_pres"))
{
  if (i==3)
  {
    next
...
saveRDS(presences_time, file="../output/presences_time_hwa.rds")
