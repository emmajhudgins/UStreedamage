...
currpopden<-as.matrix(read.csv("currpopden_5.csv", stringsAsFactors = FALSE))#historical population 
currpopden2<-as.matrix(read.csv("future_scaled_pop2.csv"))# population projections
...
for (i in 1:64) # load forecasts from Hudgins et al. 2020
{
  ...
  if (i %in% startpt==F)
  {
    pars<-read.table(paste("./new_adj_inter/par",i,"adj_inter",i, sep="."))[,1]
  }
  if (i %in% startpt==T)
  {
   pars<-read.csv(paste("./adj_inter_start/par_GDKic",i,"csv", sep="."))[,2]
  }
  if (i==48)
  {
...
saveRDS(presences_time, file="presences_time_noforce3.rds")
