## Forecast pest spread using Hudgins et al. spread model for its initial usage of species present for more than 10 years at the time of fitting
rm(list=ls()) 
require(here)
setwd(here())
presences_time<-list()
#Read in Data (same data is present in emmajhudgins/GDKvscustomized repo)
data<-read.csv('./data/countydatanorm_march.csv', stringsAsFactors = FALSE) # spatial data
data2<-read.csv('./data/spdat_clean_gdk.csv', stringsAsFactors = FALSE) # species data, see Hudgins et al. corrigendum for information on why ALB (spp=3) cannot be accurately fit
...
prez2<-read.csv('./data/prez2_clean_gdk.csv') # pest presences in 2000 for the 3 species examined here, in 2009 for all other species
L<-rep(0,64) # size of each pest's host range
ic=F # intercept correction 
for (sppp in 1:64)
{
  L[sppp]<-length(which(prez[,sppp]!=0))
}
#sources<-as.list(read.csv('Psources_notypos.csv')[,1])
currpopden<-as.matrix(read.csv("./data/currpopden_5.csv", stringsAsFactors = FALSE))#historical population 
currpopden2<-as.matrix(read.csv("./data/future_scaled_pop2.csv"))# population projections
...
currpopden<-cbind(currpopden, twenty35, currpopden2[,4], twenty45, currpopden2[,5], twenty55, currpopden2[,6]) # full population projections to 2060
...
T1<-exp(-dists/50000)
rm(dists)
YEARS<-data2$YEAR # number of years of historical spread
...
LLfit=function(par)
{
  ...
  if (temp_t==T)
  {
    temp2<-temp<-read.csv('./data/bio6_5yr.csv') 
    current_temp<-current_temp2<-temp2[,1]
  }
  hum=0
  if (hum_t==T)
  {
    hum<-read.csv('./data/interp_rel_humidity.csv')[,1]
  }
  ...
  if (i %in%startpt==F)
  {
    sources<-as.list(read.csv('./data/Psources_notypos.csv')[,1])
  }
  if (i %in%startpt==T)
  {
    load("./data/Psources_closest_march.Rdata")
  }
  Psource=sources[[spp]]
  YEAR<-YEARS[spp]
  Discovery<-2009-YEAR
...
startpt<-read.csv('./data/startpt.csv')[,1]
for (i in 1:64) # load forecasts from Hudgins et al. 2020
{
  if (i==3) # exclude ALB
  {
    next
  ...
  if (i %in% startpt==F)
  {
    pars<-read.table(paste("./data/new_adj_inter/par",i,"adj_inter",i, sep="."))[,1]
  }
  if (i %in% startpt==T)
  {
   pars<-read.csv(paste("./data/adj_inter_start/par_GDKic",i,"csv", sep="."))[,2]
  }
  if (i==48)
  {
   pars<-read.csv('./data/par_GDK_forecast.48.csv')[,1]
  }
  if (i==44)
  {
    pars<-read.csv('./data/par_GDK_forecast.44.csv')[,1]
  }
  if (i==49)
  {
    pars<-read.csv("./data/par_GDK_HWA_s_ic_c_forecast_.csv")[,2]
  }
  if (i==51)
  {
    pars<-read.csv("./data/par_GDK_GM_s_ic__forecast_.csv")[,1]
  }
  if (i==54)
  {
    pars<-read.csv("./data/par_GDK_BBD_s_ic__forecast_.csv")[,1]
  }
   LLfit(c(pars)) # forecast presences
   presences_time[[i]]<-Pfull_time
 }
saveRDS(presences_time, file="./output/presences_time_noforce3.rds")
