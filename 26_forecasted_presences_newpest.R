...
port<-c(1233,2622,3066) # major ports by trade volume (see MS)
...
#<-read.csv("hostden_clean_gdk.csv", stringsAsFactors = FALSE)
...
rm(dists)
YEARS<-data2$YEAR
pp_time<-matrix(0,3372,64)
FIA2<-read.csv('FIA_march.csv', stringsAsFactors = FALSE)
fianames<-read.csv('FIA_spp_sepcols_2019.csv')
fianames<-subset(fianames, Species%in%c("Acer", "Quercus")) #look at only maple and oak
 cols<-fianames$Code  
 pres<-rep(0, 3372)
    ...
    prez[,1]<-c(which(pres!=0),rep(0, length(which(pres==0))))
    L[1]<-length(which(pres!=0))
...
#1233 #ny
#2622 #lb
#3066 #south louisiana
for (gg in 1:3)
{
i=1
LLfit(1)
presences_time[[gg]]<-Pfull_time
}
saveRDS(presences_time, file="./output/presences_time_newpest.rds")